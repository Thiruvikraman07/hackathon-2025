...
def generate_jd(
...
) -> Tuple[Any, str, Optional[PipelineExecution]]:
    ...
    Args:
        ...
        enable_observability: Enable step-by-step execution tracking (default: True)

    Returns:
        Tuple of (JobDescription, toon_output, execution_tracking)
    """
    ...
    tracker = StepTracker(execution, verbose=verbose) if enable_observability else None
    ...
    try:
        ...
        if tracker:
            with tracker.track_step(
                ...
                input_data={"company_repo": company_repo},
                ...
                execution.repository_type = repo_type.lower()
                tracker.update_output({
                    "repository_type": repo_type,
                    "is_local": is_local_repo,
                    "detection_method": detection_method
                })
                tracker.add_decision_point(f"Repository detected as {repo_type} using {detection_method}")
        else:
            # No tracking - original logic
            repo_path_obj = Path(company_repo)
            if repo_path_obj.exists() and repo_path_obj.is_dir():
                is_local_repo = True
                repo_type = "Local"
        ...
        if tracker:
            with tracker.track_step(
                ...
                reason="Create agent with appropriate tools based on repository type"
            ):
                llm = get_chat_model("claude-3-5-sonnet")

                if is_local_repo:
                    tools = [fetch_local_repo_metadata, get_local_repo_file_structure, fetch_local_code_files]
                    tool_names = ["fetch_local_repo_metadata", "get_local_repo_file_structure", "fetch_local_code_files"]
                else:
                    tools = [fetch_repo_metadata, get_repo_file_structure, fetch_code_files]
                    tool_names = ["fetch_repo_metadata", "get_repo_file_structure", "fetch_code_files"]

                agent = create_react_agent(llm, tools=tools, response_format=JobDescription)
                ...
                tracker.add_decision_point(f"Loaded {len(tools)} tools for {repo_type} repository")
        else:
            # No tracking - original logic
            llm = get_chat_model("claude-3-5-sonnet")
            if is_local_repo:
...
Use the tools to analyze the repo, then generate ALL fields. Keep reasons concise."""
        ...
        # STEP 3: Agent Execution with LangSmith tracing
        run_id = str(uuid.uuid4())

        if tracker:
            execution.langsmith_run_id = run_id
            langsmith_project = "Synapse"  # Set explicitly as requested
            execution.langsmith_project = langsmith_project

            with tracker.track_step(
                StepType.AGENT_REASONING,
                "Agent Repository Analysis & JD Generation",
                input_data={
                    "job_title": job_title,
                    "salary_range": salary_range,
                    "prompt_length": len(prompt)
                },
                reason="Agent analyzes repository using tools and generates complete job description"
            ):
                if verbose:
                ...
                result = agent.invoke(
                    ...
                    {"run_id": run_id, "project_name": langsmith_project, "tags": ["jd-generation", execution.repository_type]}
                )

                data = result['structured_response']

                # Count tokens using tiktoken (similar to tutorial 04)
                try:
                    import tiktoken
                    ...
                    # Count input tokens (original prompt)
                    input_tokens = count_tokens(prompt)

                    # Count output tokens from all messages
                    output_tokens = 0
                    ...
                    # Calculate cost (Claude Sonnet 3.5 pricing from Bedrock)
                ...
                tracker.update_metrics(
                    files=data.total_files_analyzed,
                    tokens=tokens_used if tokens_used > 0 else None,
                    cost=cost_usd if cost_usd > 0 else None
                )
                tracker.update_output({
                    "jd_generated": True,
                    "responsibilities_count": len(data.responsibilities),
                    "qualifications_count": len(data.qualifications),
                    "experience_level": data.experience_requirement.level,
        ...
        # STEP 4: Convert to TOON and Save
        if tracker:
            with tracker.track_step(
                StepType.OUTPUT_GENERATION,
                "Generate and Save TOON Output",
                input_data={"output_format": "TOON"},
                reason="Convert JobDescription to TOON format and save to file"
            ):
                ...
                execution.output_file_path = output_file
                execution.output_size_bytes = len(output_toon)
        ...
        # Save TOON (for non-tracked mode)
        if not tracker:
            if is_local_repo:
                repo_name = repo_path_obj.name
                output_file = f"local_{repo_name}_jd.toon"
        ...
        if verbose and not tracker:
            ...
            print("💾 SAVED OUTPUT")
            print("="*80)
            print(f"✅ TOON: {output_file if not tracker else execution.output_file_path}")
...
if __name__ == "__main__":
    ...
    jd, toon, execution = generate_jd(
        ...
        enable_observability=True  # Enable step tracking
    )
    ...
    jd_local, toon_local, execution_local = generate_jd(
